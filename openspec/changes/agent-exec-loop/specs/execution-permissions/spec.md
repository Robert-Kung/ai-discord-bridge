...
The execution tier MAY run with Bash permitted inside the container ONLY when `egress-containment` is proven at startup; until then the only shell command the bridge invokes on the agent's behalf is the configured verify command. The credential-read / env-dump denies hold regardless.
...
#### Scenario: Bash relaxation requires proven egress containment
- **WHEN** the bridge starts with the exec-tier Bash relaxation enabled but `egress-containment` is not proven
- **THEN** the relaxation does not take effect (fail-closed to the pre-relaxation posture)
