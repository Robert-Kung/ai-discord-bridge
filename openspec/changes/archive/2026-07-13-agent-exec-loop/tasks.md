...
- [x] 6.1 Full pytest suite green (232); `docker compose build` + import smoke; **live smoke DONE 2026-07-09→13** — full lifecycle verified in the split deploy: job stream → diff gate → ❌ discard (no trace) / dirty-tree merge refusal / parked job survives container restart / ✅ `!merge` lands on live / `!cancel` kills via the IPC disconnect-kill contract / no-change job opens no gate. M4 live: exec-settings canary green, verify posted above the gate (real compileall pass), absent config → explicit "未設定" (rpg_battle), Bash proven (real HEAD hash + Python version written from shell output). Live finds fixed en route: worktree jobs must be sessionless (PR #15 — claude keys sessions per-cwd; resumed sids from ephemeral worktrees came back empty and poisoned the project pointer) and merge_job needed the inline committer identity (PR #16 — no gitconfig in-container)
