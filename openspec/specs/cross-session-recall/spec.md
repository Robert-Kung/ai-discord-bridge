...
## Purpose
Make decisions from past sessions reachable again after a session reset: conversation calls carry a pointer to the historical summary tree with an instruction to search it on demand (agent-side, no bridge retrieval pipeline), and written summaries carry parent-session lineage metadata for operator-side tracing.
