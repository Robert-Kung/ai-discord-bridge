...
## Purpose
Bound where the execution path can send data: routeless internal networks behind default-deny hostname allow-list proxies, proven fail-closed at startup by per-container canaries, with the phase-2 split giving each secret a single-purpose egress.
