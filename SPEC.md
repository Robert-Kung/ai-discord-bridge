...
│   │                                      例：A__home-user.json、B__home-user-my-project.json
...
### v3 待實作（本次 plan-eng-review 標的）
1. **中期 summary 改 per-(channel, cwd)**：`discord-summaries/<ch>/<cwd-slug>/`，`!cd` 換專案自動換 summary（修「切專案 summary 混淆」bug）。
2. **`!flush` 加 cwd 邊界**：只取自上次 `!cd` 以來的 transcript，不跨專案混。
3. **新增專案層 `discord-project-notes/<cwd-slug>/notes.md`** + `update_project_notes()`（Bot-B 當 worker）+ flush-before-compaction 三個寫入 trigger + 合併注入 temp 檔（帶 bot 名防 race）。
